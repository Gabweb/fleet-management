<template>
  <div class="space-y-4 p-4">
    <BasicBlock darker title="Variables">
      ...
      <ul v-else class="mt-4 space-y-2">
        <li
          v-for="key in keys"
          :key="key"
          class="flex justify-between items-center bg-slate-800 p-3 rounded-xl hover:cursor-pointer shadow-md"
          :class="{ 'border-2 border-blue-700': key === selectedKey }"
          @click="selectKey(key)"
        >
          <span>{{ key }}</span>
    ...
    <BasicBlock darker :title="selectedKey ? 'Edit Variable' : 'Select a Variable'">
      <div v-if="!selectedKey" class="p-4 text-gray-400">Select a variable to edit</div>
