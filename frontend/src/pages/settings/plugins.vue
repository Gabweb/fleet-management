    ...
    <template #header>
      <BasicBlock blurred bordered class="mb-2">
        <div class="flex justify-between items-center">
          <p class="font-semibold">Plugins</p>
          <div>
            <input
              ref="fileInput"
              type="file"
...
const { data: pluginsRaw, loading, refresh } =
  useWsRpc<Record<string, FullPluginData>>('FleetManager.ListPlugins');
...
function handleFileChange(e: Event) {
  ...
  reader.onload = () => {
    fileBase64.value = (reader.result as string).replace(/^data:.*;base64,/, '');
