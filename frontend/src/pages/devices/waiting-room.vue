    ...
    <div class="space-y-2">
        ...
        <template v-if="mode === 'pending'">
            ...
            <template v-else-if="devices">
                ...
                <div v-else :class="[small ? 'flex flex-col gap-2' : 'widget-grid']">
                    <Widget
                        v-for="(device, shellyID) in devices"
                        :key="shellyID"
                        :class="[selected.includes(shellyID) && selectMode && 'shadow-md shadow-blue-500']"
                        @click="deviceClicked(shellyID)"
                    >
                            ...
                            <div v-if="selectMode">
                                <div class="flex justify-center place-items-center mt-4 ml-2">
                                    <input
                                        ...
                                        :checked="selected.includes(shellyID)"
                                        :value="selected.includes(shellyID)"
                                    />
                                </div>
                            </div>
                            <div v-else class="flex flex-row gap-1 items-center justify-around h-[40px]">
                                <Button type="red" size="md" narrow @click="rejectDevice(shellyID)"
                                    ><i class="fas fa-xmark"
                                /></Button>
                                <Button type="green" size="md" narrow @click="acceptDevice(shellyID)"
        ...
        <template v-if="mode === 'denied'">
            ...
            <template v-else-if="deniedDevices">
                ...
                <div v-else :class="[small ? 'flex flex-col gap-2' : 'widget-grid']">
                    <Widget
                        v-for="device in deniedDevices"
                        :key="device.shellyID"
                        :class="[selected.includes(device.shellyID) && selectMode && 'shadow-md shadow-blue-500']"
                            ...
                            <div v-if="selectMode">
                                <div class="flex justify-center place-items-center mt-4 ml-2">
                                    <input
                                        type="checkbox"
                                        class="w-4 h-4 text-blue-700 bg-gray-700 border-gray-600 rounded"
                                        :checked="selected.includes(device.shellyID)"
                                        :value="selected.includes(device.shellyID)"
                                    />
                                </div>
                            </div>
                            <div v-else class="flex flex-row gap-1 items-center justify-around h-[40px]">
                                <Button type="green" size="md" narrow @click="acceptDevice(device.shellyID)"
                                    >Accept</Button
                                >
                            </div>
                        </template>
...
async function acceptDevice(shellyID: string) {
    try {
        await ws.sendRPC('FLEET_MANAGER', 'Device.AcceptPending', {
            shellyIDs: [shellyID],
        });
        refreshCorrect();
    } catch (error) {
