<template>
    <button
        ...
        @click="(event) => !loading && emit('click', event)"
    >
...
const classColor = computed(() => {
    const classes = [props.type !== 'white' ? `btn-${props.type}` : 'bg-white text-gray-800'];
    if (props.size == 'xs') {
...
.btn-orange {
    @apply bg-orange-800 from-orange-600 shadow-orange-800/50 hover:bg-orange-800 hover:from-orange-700;
}
</style>
