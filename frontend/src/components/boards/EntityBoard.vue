...
<template>
    <BoardTabs v-if="entity && device" @back="openDevice(entity.source)">
        <template #backText>
            <span class="text-gray-400 text-sm text-center cursor-pointer ml-3">Back to device</span>
        </template>
        <template #title>
        ...
        <template #default>
            <div class="mt-3">
                <div class="flex flex-col gap-3 items-center">
                    <figure
                        class="image rounded-full border w-20 h-20 flex justify-center items-center"
                        :class="[
                    ...
                    >
                        <i class="rounded-full w-16 h-16" :class="getPredefinedImageForEntity(entity.type)"
                        alt="Shelly"></i>
                    </figure>
...
import { getLogoFromModel, getPredefinedImageForEntity } from '@/helpers/device';
...
const device = computed(() => {
  return Object.values(deviceStore.devices).find(
    (d: any) => d.shellyID === entity.value.source
  );
});
