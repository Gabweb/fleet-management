...
export const useEntityStore = defineStore('entities', () => {
    ...
    function persistState() {
        localStorage.setItem('saved-entities', JSON.stringify(entities.value));
        console.debug(`🧙🏻 entity store persisted state of ${Object.keys(entities.value).length} entities.`);
    }
    ...
    // startup
    try {
        const savedEntities = JSON.parse(localStorage.getItem('saved-entities') || '{}');
        console.log(`🧙🏻 entity store loaded ${Object.keys(savedEntities).length} entities from memory.`);
        entities.value = savedEntities;
    } catch (error) {
        // ok to fail
        console.error('🧙🏻 entity store cannot parse saved entities', error);
    }
