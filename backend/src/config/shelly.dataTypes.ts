...
const floatsData = [
    ...
    .concat(x({ prefix: 'emdata:', repeat: 5, suffix: '.a_total_act_ret_energy' }))
    .concat(x({ prefix: 'emdata:', repeat: 5, suffix: '.b_total_act_energy' }))
    .concat(x({ prefix: 'emdata:', repeat: 5, suffix: '.b_total_act_ret_energy' }))
    .concat(x({ prefix: 'emdata:', repeat: 5, suffix: '.c_total_act_energy' }))
    .concat(x({ prefix: 'emdata:', repeat: 5, suffix: '.c_total_act_ret_energy' }))
    ...
    .concat(
        x({ prefix: '', repeat: 5, suffix: '' }).reduce((a: string[], v) => {
            return a.concat(
                x({ prefix: 'pm1:', repeat: 5, suffix: `.aenergy.by_minute.${v}` })
            );
        }, [])
    )
...
export const floats = {
    raw: floatsData,
    group: floatsData.map((v) => v.replace(/:\d+/i, ':*').replace(/\.\d+/i, '.*')),
};
